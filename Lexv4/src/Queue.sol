...
contract Queue is Initializable, OwnableUpgradeable, UUPSUpgradeable, IQueue{
    ...
    function drawFomoRewards() external onlyAdmin {
        ...
        require(round.count > 0, "NO_QUALIFIED_USERS");

        uint256 rewardPerUser = fomoPoolBalance * 40 / 100 / round.count; // 当前轮分40%
        ...
        // 剩余 60%留在 fomoPoolBalance 或清空
        fomoPoolBalance = fomoPoolBalance - (rewardPerUser * round.count);

        // 下一轮开始
        rounds += 1;
        Round storage nextRound = roundsData[rounds];
        nextRound.count = 0;
    }
    ...
    /////////////////////////////////////////////////////////////////////////////////////////////////
    // ================== View Functions ==================
    function getUserQueueIds(address user)
        external
        view
    ...
    function getQueueRange()
        external
        view
        returns (uint256 _head, uint256 _tail)
    {
        return (head, tail);
    }
    ...
    {
        uint256 day = _getToday(); // 你已有函数
        DailyQuota storage dq = dailyQuota[day];

        stakeUsed = dq.stakeUsed;
        unstakeUsed = dq.unstakeUsed;

        uint256 maxStake = _maxStakeQuota();
        stakeRemaining = maxStake > stakeUsed ? maxStake - stakeUsed : 0;

        uint256 maxUnstake = _maxUnstakeQuota();
        unstakeRemaining = maxUnstake > unstakeUsed ? maxUnstake - unstakeUsed : 0;
    }
    ...
    {
        Round storage r = roundsData[rounds];
